...
/*  UE_LOG(LogTemp, Warning, TEXT("language namke:%s"),
  ...
  MenuExtender->AddMenuExtension(
      "GetContent", EExtensionHook::After, nullptr,
      FMenuExtensionDelegate::CreateRaw(
          this, &FFgControlRigEditorModule::AddMenuExtension));
...
void FFgControlRigEditorModule::OnMenuEntryClicked(FMenuBuilder& MenuBuilder) {
  ...
  FString PluginBaseDir =
      IPluginManager::Get().FindPlugin(TEXT("FgControlRig"))->GetBaseDir();
  PluginBaseDir = FPaths::ConvertRelativePathToFull(PluginBaseDir);
  UE_LOG(LogTemp, Warning, TEXT("Plugin dir: %s"), *PluginBaseDir);

  FString PluginResourceDir =
      FPaths::Combine(*PluginBaseDir, TEXT("Resources"));
  ...
  MenuBuilder.AddMenuEntry(
      ...
      FUIAction(FExecuteAction::CreateRaw(
          this, &FFgControlRigEditorModule::OnImportTemplateClicked,
          ResourceFileV1)));
  ...
  //&FFgControlRigEditorModule::onFunctionTest))
  //);
}
...
void FFgControlRigEditorModule::OnMenuEntryClicked() {
  ...
  UE_LOG(LogTemp, Warning, TEXT("Unreal Engine Version: %s,%s"),
         *EngineVersion.ToString(), *EngineVersion.GetBranch());
  ...
  FString ResourceFile =
      FPaths::Combine(*PluginResourceDir, TEXT("MyResourceFile.txt"));
...
void FFgControlRigEditorModule::OnImportTemplateClicked(FString path) {
  ...
  FString actorBPPath =
      FPaths::Combine(*path, TEXT("FaceGoodActorTemp.uasset"));
  ...
  auto compilerBlueprint = [](FString path) {
    UBlueprint* LoadedBlueprint = Cast<UBlueprint>(
        StaticLoadObject(UBlueprint::StaticClass(), nullptr, *path));
    ...
    if (LoadedBlueprint) {
      ...
      FKismetEditorUtilities::CompileBlueprint(
          LoadedBlueprint, EBlueprintCompileOptions::SkipSave, &Result);
    } else {
  ...
  if (DesktopPlatform) {
    ...
    bool bResult = DesktopPlatform->OpenDirectoryDialog(
        nullptr, TEXT("Choose a directory"), DefaultPath,
        SelectedDirectoryPath);

    if (bResult) {
      // The user selected a directory
      UE_LOG(LogTemp, Warning, TEXT("Selected Directory: %s"),
             *SelectedDirectoryPath);

      // Combine the content directory path with the destination file name
      FString animBPPath_Dest = FPaths::Combine(
          *SelectedDirectoryPath, TEXT("FaceGoodAnimTemp.uasset"));
      FString ProjectDir =
          FPaths::ConvertRelativePathToFull(FPaths::ProjectDir());

      copyFileFunc(animBPPath, animBPPath_Dest);
      animBPPath_Dest = FPaths::Combine(
          *SelectedDirectoryPath, TEXT("FaceGoodAnimTemp.FaceGoodAnimTemp"));
      animBPPath_Dest = animBPPath_Dest.Replace(
          *(FPaths::Combine(ProjectDir, TEXT("Content"))), *FString("/Game"));
      compilerBlueprint(animBPPath_Dest);

      FString gamePath = SelectedDirectoryPath.Replace(
          *(FPaths::Combine(ProjectDir, TEXT("Content"))), *FString("/Game"));
      // FString actorBPPath_Dest = FPaths::Combine(*SelectedDirectoryPath,
      // TEXT("FaceGoodActorTemp.uasset")); copyFileFunc(actorBPPath,
      // actorBPPath_Dest); actorBPPath_Dest =
      // FPaths::Combine(*SelectedDirectoryPath,
      // TEXT("FaceGoodActorTemp.FaceGoodActorTemp")); actorBPPath_Dest =
      // actorBPPath_Dest.Replace(*(FPaths::Combine(ProjectDir,
      // TEXT("Content"))), *FString("/Game"));
...
void FFgControlRigEditorModule::createActorBlueprint(FString path) {
  ...
  UObject* Asset = createAsset(PackagePath, UBlueprint::StaticClass(), Factory,
  ...
  if (bp) {
    ...
    USkeletalMeshComponent* SkeletalMeshComponent =
        NewObject<USkeletalMeshComponent>(bp);

    // static ConstructorHelpers::FClassFinder<UAnimBlueprint>
    // AnimationBlueprintClassFinder(TEXT("AnimBlueprint'/Game/MetaHumans/t2/FaceGoodAnimTemp.FaceGoodAnimTemp_C'"));
    // static ConstructorHelpers::FObjectFinder<UClass>
    // AnimationBlueprintClassFinder(TEXT("AnimInstance'/Game/MetaHumans/t2/FaceGoodAnimTemp.FaceGoodAnimTemp_C'"));

    FString txtStr =
        "AnimBlueprint '" + path + "/FaceGoodAnimTemp.FaceGoodAnimTemp_C'";
    ...
    UE_LOG(LogTemp, Error,
           TEXT("Failed to add component to  Blueprint asset."));
  }
}

UObject* FFgControlRigEditorModule::createAsset(FString AssetPath,
                                                UClass* AssetClass,
  ...
  if (Factory == nullptr) {
    ...
    if (Factory == nullptr) {
      bOutSuccess = false;
      OutInfoMessage =
          FString::Printf(TEXT("Create Asset Failed - Was not able to find "
                               "factory for asset class.'%s'"),
                          *AssetPath);
      return nullptr;
    }
  }
  ...
  if (Factory->SupportedClass != AssetClass) {
    bOutSuccess = false;
    OutInfoMessage =
        FString::Printf(TEXT("Create Asset Failed - Factory cannot create "
                             "desired asset class.'%s'"),
                        *AssetPath);
    return nullptr;
  }

  UObject* Asset =
      AssetTools.CreateAsset(FPaths::GetBaseFilename(AssetPath),
                             FPaths::GetPath(AssetPath), AssetClass, Factory);
  if (Asset == nullptr) {
    bOutSuccess = false;
    OutInfoMessage =
        FString::Printf(TEXT("Create Asset Failed - Either the path is  "
                             "invalid or the aset already exists.'%s'"),
                        *AssetPath);
    return nullptr;
  }
  ...
  bOutSuccess = true;
  OutInfoMessage =
      FString::Printf(TEXT("Create Asset Succeeded - '%s'"), *AssetPath);
  return Asset;
}
